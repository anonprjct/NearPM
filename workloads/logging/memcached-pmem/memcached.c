...
//  *(((uint32_t*)(device))+255) =   (uint32_t)(((0) << 16)| 7);


  *((uint64_t*)device) = (uint64_t)(dest);
	*((uint64_t*)(device)+1) = 00;
	*((uint64_t*)(device)+2) = (uint64_t)src;
	*((uint64_t*)(device)+3) = ((uint64_t)(((0) << 16)| 7) << 32) | size;
	*(((uint32_t*)(device))+255) =   (uint32_t)(((0) << 16)| 7);
        ...
        if (c->rbytes > 0) {
            ...
            if (c->ritem != c->rcurr) {
                ...
                memmove(ch->data + ch->used, c->rcurr, tocopy);
                    ...
                    printf("a src %p dest %p\n",c->rcurr, ch->data + ch->used);
                    memmove(ch->data + ch->used, c->rcurr, tocopy);
                    
                    // printf("val %x\n",*((uint32_t*)c->rcurr));
                    // printf("val 1 %x\n",*(((uint32_t*)c->rcurr)) + 1);
                    pmem_persist(c->rcurr,tocopy);
                    pmem_persist(ch->data + ch->used,tocopy);

                    for(int i=0;i<((tocopy)/64);i++){
                      _mm_clflush(ch->data + ch->used + i*64);
                      _mm_clflush(c->rcurr + i*64);
                    }
                    //_wbinvd();
                    ...
                    pthread_mutex_lock(&counter_lock);
//                    *((uint64_t*)ndp_device) = (uint64_t)(ch->data + ch->used);  //ulog offset : destination
//                    *((uint64_t*)(ndp_device)+2) = (uint64_t)(c->rcurr); //src
//                    *((uint64_t*)(ndp_device)+3) = ((uint64_t)( ((c->objid) << 22)| ((20) << 16)| 6) <<32) | tocopy; //size
//                    _mm_mfence();
//                    *(((uint32_t*)(ndp_device))+255) =   (uint64_t)( ((c->objid) << 22)| ((20) << 16)| 6); //cmd
                    move_data((c->rcurr), (ch->data + ch->used), tocopy)
//                    wait_dma_complete();
//                    reset_dma();
                ...
                if (c->rbytes > 0) {
                    ...
                    if (c->ritem != c->rcurr) {
                        ...
                        memmove(c->ritem, c->rcurr, tocopy);
                            ...
                            printf("ab src %p dest %p oid %d\n",c->rcurr, c->ritem,c->objid);
                            memmove(c->ritem, c->rcurr, tocopy);

                            printf("val %x\n",*((uint32_t*)c->rcurr));
                            printf("val 1 %x\n",*(((uint32_t*)c->ritem)));
                            //pmem_persist(c->rcurr, tocopy);
                            //pmem_persist(c->ritem, tocopy);

                            for(int i=0;i<((tocopy)/64);i++){
                                _mm_clflush(c->ritem + i*64);
                                _mm_clflush(c->rcurr + i*64);
                            }
                            //_wbinvd();
...
//                            *((uint64_t*)ndp_device) = (uint64_t)(c->ritem);  //ulog offset : destination
//                            *((uint64_t*)(ndp_device)+2) = (uint64_t)(c->rcurr); //src
//                            *((uint64_t*)(ndp_device)+3) = ((uint64_t)( ((c->objid) << 22)| ((20) << 16)| 6) <<32) |tocopy; //size
//                            _mm_mfence();
//                            *(((uint32_t*)(ndp_device))+255) =   (uint64_t)( ((c->objid) << 22)| ((20) << 16)| 6); //cmd
                             move_data((c->rcurr), (c->ritem), tocopy)
                            // wait_dma_complete();
                            // reset_dma();    
                            //sleep(1);
                            pthread_mutex_unlock(&counter_lock);
...
static void wait_dma_complete(){
	...
	//int it =0;
    //_mm_clflush((((uint32_t*)(ndp_device))+254));
    //_mm_sfence();
    
	while((*(((uint32_t*)(ndp_device))+254))!= 1){
		asm volatile ("clflush (%0)" :: "r"((uint32_t*)(ndp_device)+254));
        _mm_sfence();
		//printf("waiting %x %x\n",*((uint32_t*)(tx->pop->p_ops.device)+11),*((uint32_t*)(tx->pop->p_ops.device)+254));
		//printf("waiting read val %d!!\n",(*(((uint32_t*)(ndp_device))+254)));
        //int a = (*(((uint32_t*)(ndp_device))+254)) + 1;
		//it++;
	}
...
int main (int argc, char **argv) {
    ...
    void * pool1;
    void * pool2;
    char str[80] = "/mnt/mem/pool1";
    char str1[80] = "/mnt/mem/pool2";


    if ((pool1 = pmem_map_file(str, 4096,
        PMEM_FILE_CREATE, 0666, &mapped_len, &is_pmem)) == NULL) {
        fprintf(stderr, "pmem_map_file failed\n");
        exit(0);
    }

    uint64_t* tmp = (uint64_t*)pool1;
	...
	*tmp = 0;
    ...
    if ((pool2 = pmem_map_file(str1, 4096,
        PMEM_FILE_CREATE, 0666, &mapped_len, &is_pmem)) == NULL) {
        fprintf(stderr, "pmem_map_file failed\n");
        exit(0);
    }
	...
	*tmp = 1;
    ...
    void *a = malloc(128*1024*1024);
    
    memset(pool1,2,4096);
    memset(pool2,3,4096);
    printf("pool1 %x pointer %p\n",*((uint32_t*)pool1),pool1);
    printf("pool2 %x pointer %p\n",*((uint32_t*)pool2),pool2);


    //memset(a,34,128*1024*1024);
   // for(int i=0;i<((4096)/64);i++){
   //     _mm_clflush(pool2 + i*64);
   // }
   // for(int i=0;i<((4096)/64);i++){
   //     _mm_clflush(pool1 + i*64);
   // }
   // _mm_mfence();
    
    //pmem_persist(pool2,4096);
    //
    *((uint64_t*)ndp_device) = (uint64_t)(pool1);  //ulog offset : destination
    *((uint64_t*)(ndp_device)+2) = (uint64_t)(pool2); //src
    *((uint64_t*)(ndp_device)+3) = ((uint64_t)( ((1) << 22)| ((0) << 16)| 6) <<32) |1024; //size
    _mm_mfence();
    *(((uint32_t*)(ndp_device))+255) =   (uint64_t)( ((1) << 22)| ((0) << 16)| 6); //cmd
